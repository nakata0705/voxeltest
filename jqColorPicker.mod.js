(function(window, $, Colors, undefined){
	...
	var $document = $(document),
		...
		_html = '<div class="cp-color-picker"><div class="cp-z-slider"><div c' +
			...
			'class="cp-alpha-base"><div class="cp-alpha"><div class="cp-alpha-cursor"></div></div></div></div>',
			// 'grunt-contrib-uglify' puts all this back to one single string...
		_css = '.cp-color-picker{position:relative;overflow:hidden;padding:6p' +
			'x 6px 0;background-color:#444;color:#bbb;font-family:Arial,Helve' +
			'tica,sans-serif;font-size:12px;font-weight:400;cursor:default;' +
	...
	function preRender(toggled) {
		...
		_$alpha._css = {background: 'linear-gradient(to right,rgba(' + RGBInnerText + ',0),rgba(' + RGBInnerText +',1)' +')'};
		_$alpha_cursor._css = {
	...
	$.fn.colorPicker = function(options) {
		...
		on(_pointerup, _selector, toggle).
